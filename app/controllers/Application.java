...
public class Application extends Controller {
	...
	public static void my_xingcard(){
		long uid = Long.parseLong(session.get("uid"));
		List<OrderItem> items = JPA.em().createNativeQuery("select * from order_item item right join order_pay pay on pay.id = item.order_id where pay.user_id=:uid", OrderItem.class).setParameter("uid", uid).getResultList();
        render(items);
	}
